...
function ViewApplications() {
  ...
  <div className='flex items-center justify-center h-[70vh]'>
    <p className='text-xl text-red-400 sm:text-2xl'>No Application Received or Available</p>
  </div>
  ) : (
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
              <tr key={index} className={`text-gray-700 hover hover:bg-gray-300 ${index % 2 === 0 ? 'bg-gray-200' : ''}`}>
                ...
                <td className='py-2 px-4 border-b flex'>
                  ...
                  <span className='mt-3'>{applicant.userId.name}</span>
                </td>
                ...
                <td className='px-4 py-2 mb-3 fiex items-center'>
                  <a href={applicant.userId.resume} target='_blank' className=' text-blue-400 px-3 py-1 rounded inline-flex gap-2 text-left items-center'>
                  ...
                  {applicant.status === "Pending"
                    ? <div className='relative inline-block text-left group'>
                      ...
                      <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow group-hover:block'>
                        <button onClick={() => changeJobApplicationStatus(applicant._id, 'Accepted')} className='block w-full text-left px-4 py-2 text-sky-500 hover:bg-gray-100'>Accept</button>
